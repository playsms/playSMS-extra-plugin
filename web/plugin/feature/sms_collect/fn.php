 ...
 */
function sms_collect_hook_checkavailablekeyword($keyword) {
	$ok = true;
	$db_query = "SELECT collect_id FROM " . _DB_PREF_ . "_featureCollect WHERE collect_keyword='$keyword'";
 ...
 *   check if keyword is for sms_collect
 * @param $collect_param
 ...
 */
function sms_collect_hook_setsmsincomingaction($sms_datetime, $sms_sender, $collect_keyword, $collect_param = '', $sms_receiver = '') {
	$ok = false;
	$db_query = "SELECT uid FROM " . _DB_PREF_ . "_featureCollect WHERE collect_keyword='$collect_keyword'";
	$db_result = dba_query($db_query);
	if ($db_row = dba_fetch_array($db_result)) {
		$c_uid = $db_row['uid'];
		if (sms_collect_handle($c_uid, $sms_datetime, $sms_sender, $collect_keyword, $collect_param, $sms_receiver)) {
			$ok = true;
		}
	}
	$ret['uid'] = $c_uid;
...
function sms_collect_id2keyword($collect_id) {
	if ($collect_id) {
